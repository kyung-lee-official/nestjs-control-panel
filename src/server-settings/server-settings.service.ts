...
export class ServerSettingsService {
	 ...
	 */
	async create(createServerSettingDto: CreateServerSettingDto): Promise<ServerSetting> {
		const serverSetting = this.settingsRepository.create(createServerSettingDto);
	...
	async isSignUpAvailable(): Promise<{ isSignUpAvailable: boolean; }> {
		...
		if (dbSettings.length !== 1) {
			throw new InternalServerErrorException("Server setting number is not 1");
		}
	...
	async findAll(): Promise<ServerSetting> {
		const settingQb = this.settingsRepository.createQueryBuilder("serverSetting");
		settingQb.limit(1);
	...
	async update(updateServerSettingDto: UpdateServerSettingDto): Promise<ServerSetting> {
		const { allowPublicSignUp, allowGoogleSignUp } = updateServerSettingDto;
		const settingQb = this.settingsRepository.createQueryBuilder("serverSetting");
		settingQb.limit(1);
		...
		if (allowGoogleSignUp !== null && allowGoogleSignUp !== undefined) {
			setting.allowGoogleSignUp = allowGoogleSignUp;
		}
		console.log(setting);
