...
export class GroupsService {
	...
	async update(id: number, updateGroupDto: Partial<UpdateGroupDto>): Promise<Group> {
		...
		try {
			ForbiddenError.from(ability).throwUnlessCan(Actions.UPDATE, dbGroup);
		} catch (error) {
			if (error instanceof ForbiddenError) {
				throw new BadRequestException(error.message);
			}
		...
		if (ownerId && userIds) {
			...
			const dbUsers = await this.usersRepository.find({ where: { id: In(userIds) } });
	...
	async remove(id: number): Promise<any> {
		...
		try {
			ForbiddenError.from(ability).throwUnlessCan(Actions.DELETE, dbGroup);
		} catch (error) {
			if (error instanceof ForbiddenError) {
				throw new BadRequestException(error.message);
			}
		...
		if (dbGroup.users.length > 0) {
			throw new BadRequestException("Can not delete a group that has users");
		}
